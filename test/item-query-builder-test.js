...
describe('search', function(){
  ...
  it('should filter by date', function(done) {
    ...
    store.Item.saveList(items).then(function(items) {
      ...
      // filter for weather
      search.itemQueryBuilder({before: Date.now()}, function(err, results) {
        assert.isNull(err);
        assert(results.length===3);

        search.itemQueryBuilder({after: Date.now()}, function(err, results) {
          assert.isNull(err);
          assert(results.length===1);

          search.itemQueryBuilder({
  ...
  it('should filter by license', function(done) {
    ...
    store.Item.saveList(items).then(function(items) {
      assert(items.length === 3);
      // filter for weather
      search.itemQueryBuilder({license: 'odbl,commercial'}, function(err, results) {
        assert(results.length===2);
        done();
      });
    }, function(err) {
  ...
  it('should filter by lifespan', function(done) {
    ...
    store.Item.saveList(items).then(function(items) {
      assert(items.length === 3);
      // filter for weather
      search.itemQueryBuilder({lifespan: 'temporary'}, function(err, results) {
        assert(results.length===1);
        done();
      });
    }, function(err) {
  ...
  it('should filter by coords', function(done) {
    ...
    store.Item.saveList(items).then(function(items) {
      ...
      search.itemQueryBuilder({location: '-97.743644, 30.272922'}, function(err, results) {
        assert.isNull(err);
